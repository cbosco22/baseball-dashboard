...
league_filter = st.sidebar.multiselect("League", options=data['LeagueAbbr'].unique(), default=data['LeagueAbbr'].unique())
team_filter = st.sidebar.multiselect("Team/School", options=data['teamName'].unique(), default=data['teamName'].unique())
...
if sort_stat != 'None':
    filtered_data = filtered_data.sort_values(sort_stat, ascending=(sort_stat in ['ERA']))  # Ascending for ERA (lower better), descending others
...
# Map: Hot zones by state (choropleth heatmap)
